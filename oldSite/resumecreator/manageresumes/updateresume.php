...
<?php
  ...
  $resume = "";
  $sql = "
	SELECT * FROM `Resumes` WHERE (`userID`='$userID') AND (`id`='$ID')
  ";
  $result = db_get($sql);
  if(@mysql_num_rows($result) == 1) {
	...
	header_setTitle("Update Resume -- Error");
	generateHeader();
  ...
  generateHeader();
?>
  <form name='updateresume' action="<?php echo $_SERVER['PHP_SELF']?>?id=<?php echo $ID?>" method='post'>
 <center>
 ...
 <br />
  ...
  <tr>
   ...
   <td nowrap='true' width='100%'><input name='currentPhone' value='<?php echo $resume['currentPhone']?>' /> (541) 123-4567</td>
  </tr>
  <tr>
   <td nowrap='true' valign='top'><strong>Alternate Phone: </strong></td>
   <td nowrap='true' width='100%'><input name='perminantPhone' value='<?php echo $resume['perminantPhone']?>' /> (503) 123-4567</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td nowrap='true' valign='top' width='100%'><textarea name='employment' rows='18' cols='60'><?php echo $resume['employment']?></textarea></td>
   <td valign='top' nowrap='true'>
