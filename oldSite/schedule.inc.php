...
/*
	...
	------------------------------------------------------------------------------------- 
*/
function schedule_generate($classes_arr, $options, $conn) {
		$javascript  = "<script language=\"javascript\">\r\n";
		$javascript .= "  <!--\r\n"; 
		$javascript .= "    function showTutor( TutorID ) {\r\n"; 
		$javascript .= "      w = window.open(\"showtutorinfo.php?tutorid=\"+TutorID+\"\", '', 'scrollbars=yes, width=350, height=350, resizable=no');\r\n";
		$javascript .= "      w.focus();\r\n"; 
		$javascript .= "    }\r\n"; 
		$javascript .= "  // -->\r\n"; 
		$javascript .= "</script>\r\n";
		...
		if ( intval($options["row_interval"]) == 0 )
			$options["row_interval"] = 30;	
		...
		$html = "". $javascript ."<table width=\"100%\" bgcolor=\"#000000\" cellspacing=\"1\" >\n";
...
*/		
		$cell_style = "background-color: #c0c0c0;"; // default day header style
		...
		while ($cur_time < $options["end_time"])
		{
			...
			for ($cur_day = 0; $cur_day < 7; $cur_day++)
			{
				...
				if ( isset($classes_arr[$cur_day][$cur_time]["name"]) ) 
				{
					$class_interval = intval($classes_arr[$cur_day][$cur_time]["length"]);
					if ( $class_interval == 0 )
						$class_interval = 60; // default interval is 60mins
					...
					$name =  $classes_arr[$cur_day][$cur_time]["name"];
					$ID   = $classes_arr[$cur_day][$cur_time]["ID"];
					$html .= "		<td width=\"14%\" rowspan=\"$class_span\" style=\"$cell_style\"><a href=\"javascript:showTutor('".$ID."');\"><b>".$name."</b></a></td>\n";
				}		
				else 
				{
					$html .= "		<td width=\"14%\">&nbsp;</td>\n";		
		...
		$sql = "SELECT * FROM Tutoring LEFT JOIN Times ON Tutoring.TutorID = Times.TutorID WHERE (TimesID IS NULL) ORDER BY Tutoring.TutorID";
                ...
                while($row = mysql_fetch_assoc($result))
                {
                        if($first)
                        {
                                $html .= "<H2>The following people do not have set times but will be in the Hovland Lab and will be willing to help:</H2><BR><UL>\n";
                                $first = false;
                        }
                        $html .= $row["Name"] . "<BR>\n";
                }
